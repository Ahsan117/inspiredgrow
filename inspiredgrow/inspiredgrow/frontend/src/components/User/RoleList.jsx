  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        ...
        setError("Failed to fetch roles. Please try again later."); // Display error on UI
      }
    };
    fetchRoles();
  }, []);
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-grow">
        ...
        <div className="container mx-auto py-10 p-10">
          ...
          {error && (
            <div className="mb-4 text-red-500">{error}</div> // Display error message if fetch fails
          )}

          {roles.length === 0 && !error ? (
            <div className="text-center text-gray-500">No roles available.</div> // Show empty state message
          ) : (
                ...
                {currentRoles.map((role, index) => (
                  <tr key={role._id} className="border border-black">
                    ...
                    <td className="px-4 py-2 border border-black">
                      <button
                        className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                        onClick={() => navigate(`/roles/${role._id}`)} // Action button linking to role details
                      >
                        View
                      </button>
                    </td>
